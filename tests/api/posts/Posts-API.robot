...
7 - Upload com Pins
    ...
    ...    whatsapp_number=${EMPTY}
    @{signers_para_envio}=    Create List    ${signer_por_email}
    ${resp2}=    Adicionar Signatarios    ${UUID_DOCUMENT}    ${TOKEN_API}    ${CRYPT_KEY}    @{signers_para_envio}
...
9 - Envio com edição e remoção de signatário
    ...
    ...    whatsapp_number=${EMPTY}
    @{signers_para_envio}=    Create List    ${signer_por_email}
    ${response_add}=    Adicionar Signatarios    ${UUID_DOCUMENT}    ${TOKEN_API}    ${CRYPT_KEY}    @{signers_para_envio}
    Status Should Be    200    ${response_add}
    ${json_add}=    Call Method    ${response_add}    json
